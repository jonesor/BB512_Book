...
Before we dig into the formal LTRE, it is useful to compare the two matrices, `A_ref` and `A_treat`, to see how they are different from each other.
...
- Modify one element in `A_treat` and predict which LTRE contribution will change most.
- Re-run `popbio::LTRE` and check whether your prediction matches the output.
