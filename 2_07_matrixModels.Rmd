...
```{r matrix_chapter_packages_hidden, include = FALSE, warning = FALSE, message = FALSE}
library(tidyverse)
library(Rage)
library(Rcompadre)
```
...
```{r matrix_chapter_diagram_packages_hidden, include = FALSE, warning = FALSE, message = FALSE}
library(rsvg)
...
If you are running this chapter in a fresh R session, install and load required packages first.

```{r matrix_package_setup, eval = FALSE, echo = TRUE}
# Install once (only if not already installed)
install.packages(c("popdemo", "Rage", "Rcompadre", "tidyverse", "rsvg", "DiagrammeRsvg"))
...
In my example, I have 3 stages, so I have 3 values for the initial population sizes. Then I use the `popdemo` function `project` to do a population projection for 10 years (10 time steps). Each step represents one annual cycle of survival, growth, and reproduction.
...
Because our projection interval is one year, λ represents the annual population growth rate. You can see that in this case, using my example values the population is growing `r format(100*(eigs(A)$lambda-1),2,4)`% per year.
