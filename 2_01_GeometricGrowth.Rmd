...
1. Use the geometric growth equation: $N_t = N_0 \lambda^t$.
2. Substitute values: $N_5 = 10 \times 1.1^5$.
...
6. Add a trendline with an equation to the log transformed graph. Identify the slope. The slope is $ln(\lambda)$), so you can work out $\lambda$ by "reversing" the $ln$ by taking its exponential. e.g. `=EXP(0.0953)` in Excel.


Now reset the population growth rate to 1.1. Now let's see if the mathematical rules of so-called “geometric series” work...
