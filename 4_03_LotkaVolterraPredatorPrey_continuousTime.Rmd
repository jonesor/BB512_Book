...
We will use the `deSolve` package to solve the system of differential equations numerically. Note that you may need to install the package first using the command `install.packages("deSolve")`.

```{r loadlib, include = TRUE, echo = TRUE}
library(deSolve)
```
...
```{r population-dynamics, echo = TRUE}
...
2. **Neutral Stability**: The oscillations are neutrally stable under idealized conditions; they persist indefinitely without damping or amplification unless external factors or parameter changes are introduced. Unless the parameters are set to truly extreme values, extinction is not possible.
...
5. **Phase Plane Behaviour**: Predator and prey populations follow closed orbits in the phase plane around the coexistence equilibrium, under the assumption that the system remains undisturbed.

## Takeaways

- Continuous-time predator-prey models generate sustained cycles under ideal conditions.
- Equilibria and isoclines help explain why cycles form and how parameters shift them.
