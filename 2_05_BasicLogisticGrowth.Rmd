...
- **Step 1**: Examine the default settings in Graph 1, where `Initial N` = 10, `r_m` = 0.8, and `K` = 200.  
  ...
  - What is the maximum population size in relation to the carrying capacity?  
    <!-- The maximum population size should be equal to the carrying capacity, so around 200. -->
  - At what time does the population reach its maximum size?  
    <!-- The population reaches its maximum size relatively quickly, around 20–25 time steps. -->
...
- **Step 3**: Halve the growth rate (`r_m`) to 0.4.  
  ...
  **Test**: What do you observe in the graph?  
  <!-- The population takes about twice as long to reach the carrying capacity (roughly 40–50 time steps). -->
...
- **Step 4**: Increase `r_m` to 1.8.  
  ...
  - How does the maximum population size compare to the carrying capacity?  
    ...
    <!-- The population temporarily exceeds the carrying capacity, showing oscillations above and below `K`. -->
...
- **Step 5**: Experiment with higher values for `r_m` (e.g., 2.0, 2.8, 2.9, 3.0).  
  ...
  - What happens when `r_m` is 2.8 or 3.0? Are the dynamics predictable or chaotic?  
  ...
  - Is there a limit to how high `r_m` can go before the population crashes or becomes unsustainable?  
    
    <!-- When `r_m` is too high, the population may oscillate chaotically and could potentially go extinct if the oscillations drop the population size below zero. -->

- **Step 6**: Compare the population trajectory for populations with `r_m` = 2.8 and `r_m` = 2.81, and then fix `r_m` at 2.8 but vary the initial population size by a small amount (e.g., +1).  
...
- **Step 4**: Experiment with different values for `K` and `r_m`.  
  ...
  - Can you predict what the graph would look like if the growth rate were negative (`r_m < 0`)?  

    <!-- If `r_m` is negative, the y-intercept will be below the x-axis, and the population will shrink over time. -->
