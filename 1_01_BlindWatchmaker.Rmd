...
The R code below doesn't just sample from the 26 letters of the alphabet plus a space: it also allows a few extra symbols (an apostrophe, an exclamation mark, and the Danish letters Å, Æ and Ø), giving a pool of 32 possible characters at each position. The chance of randomly typing the first letter correctly is 1 in 32. The probability of getting the first two letters right is \((1/32)^2\), and the chances of typing the entire 14-character sentence correctly by random typing alone is \((1/32)^{14} = 8.47 \times 10^{-22}\), an astronomically small number.
...
- Selection can rapidly amplify small, random improvements when there is a consistent criterion for success.
- Random variation alone is extremely unlikely to generate complex outcomes, but selection changes the odds.
- Real natural selection has no target; it favors traits that improve fitness in the current environment.
