...
Set $r_m = 1.2$ and $K = 200$ in the `BasicLogistic` sheet. You should see a smooth approach to $K$ in Figure 1, a straight decreasing line in Figure 2 that crosses zero at $K$, and a single peaked curve in Figure 3 with a maximum near $N = K/2$.
...
1. In the **`TimeLag` worksheet**, explore how adding a time lag to the logistic model affects population dynamics. This is the equation we are using: $\frac{d N}{d t}=r N\left(1-\frac{N_{t-\tau}}{K}\right)$
...
- The shape of $dN/dt$ and per-capita growth curves explains why oscillations occur.
