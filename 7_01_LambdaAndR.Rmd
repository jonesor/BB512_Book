...
|  $R_0$   | Net reproductive rate: the expected number of offspring per individual, per generation ($\sum l_x m_x$)  |  Net Reproductive Rate |
...
```{r plotprojections, echo = TRUE, message = FALSE}
...
```{r plotnewproj, message = FALSE, echo = TRUE}
...
```{r plotproj2, message = FALSE, echo = TRUE}
df1$logN <- log(df1$N)
...
```{r plotproj2_summary, echo = FALSE}
summary(mod1)
```
